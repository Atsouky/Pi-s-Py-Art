...
def initialiserCellules():
    return [[None for i in range(nbCellHeight+1)] for j in range(nbCellWidth+1)]
...
def prochaine_vie(vie):
    ...
    for x in range(nbCellWidth):
        for y in range(nbCellHeight):
            ...
            if vie[x][y] is not None:
                #for dx, dy in [ (-1, 0), (0, -1), (0, 1), (1, 0), (-1, -1), (-1, 1), (1, -1), (1, 1)]:
                for dx, dy in [ (-1, 0), (0, -1), (0, 1), (1, 0)]:
        
                    nx, ny = (x + dx) % nbCellWidth, (y + dy) % nbCellHeight
                    if vie[nx][ny] is None:
                        if random() < chance_mutation:
                            
                            
                            newCell.append((nx,ny,mutation(vie[x][y])))
                        """else :
                            newCell.append((nx,ny,vie[x][y]))"""
                        
    for nx,ny,ncolo in newCell:
        vie[nx][ny]=ncolo
