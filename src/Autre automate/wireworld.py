...
def remplirGrille(vie):
    ...
    for x in range(nbCellWidth):
        for y in range(nbCellHeight):
            if vie[x][y]==1:
                pygame.draw.rect(fenetre, (100,100,100), (x*CELLSIZE, y*CELLSIZE, CELLSIZE, CELLSIZE))
            elif vie[x][y] == 2:
...
def voisins(item,vie):
    ...
    return nbVoisins
    '''
def voisin(x, y, vie):# 0 vide 1 cable 2 tete 3 queue
    nbvoisin = 0
    nbvoisin1 = 0
    for dx in [-1, 0, 1]:
        for dy in [-1, 0, 1]:
...
def prochaine_vie(vie):
    ...
    for x in range(nbCellWidth):
        for y in range(nbCellHeight):
            b,a = voisin(x, y, vie)# 0 vide 1 cable 2 tete 3 queue
            ...
            if vie[x][y] == 0:next_vie[x][y] = 0
            elif vie[x][y] == 2:next_vie[x][y] = 3
            elif vie[x][y] == 3:next_vie[x][y] = 1
...
while loop==True:
    ...
    if mousepos[0]<nbCellWidth*CELLSIZE and mousepos[1]<nbCellHeight*CELLSIZE: 
        if mousePressed1:
            ...
            vie[mousepos[0]//CELLSIZE][mousepos[1]//CELLSIZE]=selecteur
        if mousePressed2:
